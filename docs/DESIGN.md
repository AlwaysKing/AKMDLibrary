...
1. `docs/` 下的直接子文件夹 = **Space**（`public/` 除外）
2. `.md` 文件 = **Page**
3. 与 `.md` 同名的文件夹 = 该 Page 的**子页面目录**
4. `public/` = 当前目录层级页面引用的**资源**（图片/视频等），不作为页面或空间处理
5. 子页面目录内的 `.md` = 子 Page（**递归**，每层都可有 `public/`）
6. 以 `.` 开头的文件/文件夹忽略（如 `.DS_Store`）
7. MD 中用相对路径引用资源：`![](public/uuid1/uuid2/图片.png)`
...
GET    /api/spaces/:slug/pages/:id/assets/*   ← 服务页面 public/ 下的资源
```
...
POST   /api/upload                  ← 上传图片/文件
       → 保存到对应页面目录的 public/{uuid}/{uuid}/文件名
       → 返回相对路径用于 MD 中引用
...
- **public/ 资源服务**：Go 静态文件服务，映射 `public/` 路径到实际文件系统
