...
import { Link, Bookmark, AtSign } from 'lucide-react';
...
export default function LinkPasteMenu({ url, position, onInsertLink, onInsertBookmark, onInsertMention, onClose }: LinkPasteMenuProps) {
  ...
  return createPortal(
    ...
    >
      {mode === 'menu' && (
        <>
          <div className="px-2.5 py-1 text-xs text-notion-textSecondary font-medium">
            粘贴为
          </div>
          <button
          ...
          <button
            onClick={() => onInsertMention(url)}
            className="w-full flex items-center gap-2.5 px-2.5 py-1.5 text-sm text-notion-text hover:bg-notion-hover rounded-md transition-colors"
          >
            <AtSign className="w-4 h-4 text-notion-textSecondary" />
            提及
          </button>
          <button
